...
def get_5_day():
    ...
    for i in range(len(raw['dwml']['data']['parameters'])):
        all_data.append({'city': raw['dwml']['data']['location'][i]['city']['#text'],
...
db = client.weather_site_forecast
collection = db.forecast_5day
...
collection.find_one({'city': 'Oklahoma City'})

all_cities = collection.find({},{'_id': 0, 'city':1})
